  ...
  const NavContent = () => (
    <div className="flex flex-col h-full">
      ...
      <div className="px-5 py-5 border-b border-gray-100">
        ...
        <p className="font-semibold text-gray-900 text-sm mt-1 leading-tight">{business.name}</p>
        <p className="text-xs text-gray-400 capitalize">{business.plan.replace('_', ' ')}</p>
      </div>
      ...
      <div className="px-3 pb-3">
        <Link
          href="/scanner"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-indigo-600 bg-indigo-50 hover:bg-indigo-100 transition-colors"
        >
    ...
    <>
      ...
      {mobileOpen && (
        <>
          ...
          <aside className="lg:hidden fixed inset-y-0 left-0 z-40 w-64 bg-white">
            <div className="pt-16">
              <NavContent />
            </div>
          </aside>
